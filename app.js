...
let device,server,service,statusChr,radarChr,gpsChr,cmdChr,otaCtrlChr,otaDataChr,otaStatChr;
...
async function connectBle(){try{
 ...
 [statusChr,radarChr,gpsChr,cmdChr,otaCtrlChr,otaDataChr,otaStatChr]=await Promise.all([
  ...
  service.getCharacteristic(U.otaCtrl),service.getCharacteristic(U.otaData),service.getCharacteristic(U.otaStat)
 ]);
...
function onDisc(){$("bleDot").className="dot off";$("bleState").textContent="Mất kết nối";$("connect").disabled=false;["disconnect","readcfg","writecfg","fw","ping","otaStart","otaAbort"].forEach(id=>$(id).disabled=true);$("otaState").textContent="Đã mất kết nối BLE.";otaRunning=false}
...
async function otaUpload(){
 ...
 const f=$("fwFile").files[0];if(!f)return alert("Chọn file .bin trước.");
 ...
 otaRunning=true;$("otaStart").disabled=true;$("otaAbort").disabled=false;
 $("otaState").textContent="Đang khởi tạo OTA...";$("otaBar").style.width="0%";

 await otaCtrlChr.writeValue(enc.encode(`START {"size":${f.size},"crc32":${crc}}`));
 await new Promise(r=>setTimeout(r,250));

 const bytes=new Uint8Array(buf);
 let chunk=180; // fast default; auto-fallback if browser/MTU rejects
 let sent=0,started=performance.now(),lastUi=started;

 while(sent<bytes.length){
   if(!otaRunning)throw new Error("Đã hủy OTA");
   let n=Math.min(chunk,bytes.length-sent),part=bytes.slice(sent,sent+n);
   try{
     if(otaDataChr.writeValueWithoutResponse) await otaDataChr.writeValueWithoutResponse(part);
     else await otaDataChr.writeValue(part);
   }catch(err){
     if(chunk>20){chunk=chunk>128?128:chunk>64?64:20;log(`Giảm chunk BLE xuống ${chunk} byte`);continue}
     throw err;
   }
   sent+=n;

   // Yield periodically to avoid overrunning the BLE stack.
   if((sent/chunk)%12===0) await new Promise(r=>setTimeout(r,2));

   const now=performance.now();
   if(now-lastUi>120||sent===bytes.length){
     lastUi=now;
     const sec=(now-started)/1000,rate=(sent/1024)/Math.max(sec,.001),pct=100*sent/bytes.length;
     $("otaBar").style.width=pct+"%";$("otaPercent").textContent=pct.toFixed(1)+"%";
     $("otaSpeed").textContent=rate.toFixed(1)+" KB/s";$("otaBytes").textContent=(sent/1024).toFixed(1)+" KB";$("otaTime").textContent=sec.toFixed(1)+" s";
   }
 }
 await otaCtrlChr.writeValue(enc.encode("END"));
 const sec=(performance.now()-started)/1000;
 log(`OTA data xong: ${(bytes.length/1024).toFixed(1)} KB / ${sec.toFixed(1)} s = ${((bytes.length/1024)/sec).toFixed(1)} KB/s`);
 $("otaState").textContent="Đã gửi xong, đang kiểm tra CRC32 và kích hoạt firmware...";
}
